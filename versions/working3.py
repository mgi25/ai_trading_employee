...
HEDGE_STEP_PIPS = 150    # want 20 pips per opposite-side hedge
STEP_PRICE      = PIP_SIZE * HEDGE_STEP_PIPS  # -> 0.20
GRID_TOLERANCE  = PIP_SIZE * 3 
# === RECOVERY SNIPER SETTINGS ===
# === CONFIGURATION ===
SPREAD_LIMIT = 1.5
PROFIT_SCALP_TARGET = 0.5
HEDGE_TRIGGER_PIPS = 150 
...
def get_lux_ranges(df_bars, length=8, mult=0.9, atr_len=300, min_duration=3):
...
def current_lux_box(ranges, now):
    # Make sure 'now' is naive, or convert all to UTC
    # Best is: force all to UTC-aware
    if now.tzinfo is None or now.tzinfo.utcoffset(now) is None:
        now = now.tz_localize('UTC') if hasattr(now, 'tz_localize') else pd.Timestamp(now, tz='UTC')
    for r in ranges:
        # Convert both start/end to UTC if not already
        start = r['start']
        end = r['end']
        ...
        if start <= now <= end + pd.Timedelta(minutes=1):
            return r
    return None
...
def reset_state():
    ...
    global first_recovery_price, last_recovery_leg, last_recovery_price, last_recovery_side  # <-- PATCH
    ...
    lock_anchor_price = None
    lock_anchor_side = None
    first_recovery_price = None         # <-- PATCH
    last_recovery_leg = 0               # <-- PATCH
    last_recovery_price = None          # <-- PATCH
    last_recovery_side = None           # <-- PATCH
    cancel_all_pendings()
...
# ─────────────────────────────────────────────────────────────
def must_panic_exit_recovery(tick):
    ...
    if len(rec_positions) < 3:
        return False  # Need at least 3 legs first

    last_side = 'BUY' if rec_positions[-1].type == mt5.POSITION_TYPE_BUY else 'SELL'
    next_side = 'SELL' if last_side == 'BUY' else 'BUY'
    if base_entry_price is None:
        return False

    # Calculate where the 4th leg would be
    target = recovery_grid_price(base_entry_price, 4, REC_PIPS * PIP_SIZE)
    quote = tick.bid if next_side == 'SELL' else tick.ask
...
def track_pending(res, comment, side):
    """
    Call immediately after place_pending/ order_send.
    Caches the order so already_pending() sees it for a short grace window.
    """
    if res and res.retcode == mt5.TRADE_RETCODE_PLACED:          # 10023
        pending_watchlist[res.order] = {"comment": comment, "side": side}
        recently_sent[res.order] = {"ts": time.time(),           # ← std-lib time
                                    "comment": comment,
                                    "side": side}
...
def price_between_last_buy_sell(entry_sequence, price):
    """
    Returns True if price is strictly between the last BUY and last SELL entries,
    regardless of their absolute values.
    """
    last_buy = next((e[0] for e in reversed(entry_sequence) if e[1] == 'BUY'), None)
    last_sell = next((e[0] for e in reversed(entry_sequence) if e[1] == 'SELL'), None)
    if last_buy is None or last_sell is None:
        return False
    low, high = sorted([last_buy, last_sell])
    return low < price < high

def sync_pendings():
    global last_recovery_price, last_recovery_side, last_recovery_leg  # <-- ADD THIS AT THE TOP
    positions = mt5.positions_get(symbol=SYMBOL) or []
    to_remove = []
    for oid, info in pending_watchlist.items():
        order = mt5.orders_get(ticket=oid)
        if not order:  # gone (filled or canceled)
            # check if it produced a position (optional heuristic)
            for pos in positions:
                pos_side = 'BUY' if pos.type == mt5.POSITION_TYPE_BUY else 'SELL'
                if info['comment'].startswith("RECOVERY") and pos_side == info['side']:
                    last_recovery_price = pos.price_open
                    last_recovery_side  = pos_side
                    try:
                        last_recovery_leg = int(info['comment'].split('_')[1])
                    except Exception:
                        last_recovery_leg = 1
            to_remove.append(oid)        # <── mark for deletion
    ...
    rebuild_state()  # update current_leg etc.
...
def recovery_grid_price(anchor_price, lock_anchor_side, leg, step_pips=1.0):
    """
    Returns the exact grid price for each recovery leg (1-based index).
    - Leg 1: anchor_price (opposite side)
    - Leg 2: anchor ± step (same as lock side)
    - Leg 3: anchor (opposite side)
    - Leg 4: anchor ± 2*step (should NOT place, trigger full exit)
    - ...continue if needed (but 4th triggers exit as per your logic)
    """
    sign = 1 if lock_anchor_side == 'BUY' else -1

    if leg == 1 or leg == 3:
        # Odd legs: always anchor price
        return round(anchor_price, 3)
    elif leg == 2:
        # Even leg 2: anchor ± step_pips
        return round(anchor_price + sign * step_pips, 3)
    elif leg == 4:
        # Even leg 4: anchor ± 2*step_pips (should not actually trade)
        return round(anchor_price + sign * 2 * step_pips, 3)
    else:
        raise ValueError("Invalid recovery leg (should be 1, 2, 3, or 4)")
def move_all_recovery_sls_to_leg4_price():
    if first_recovery_price is None or lock_anchor_side is None:
        logging.warning("Cannot set SL: missing first_recovery_price or lock_anchor_side")
        return

    leg4_price = recovery_grid_price(first_recovery_price, lock_anchor_side, 4, STEP_PRICE)
    ...
    for pos in mt5.positions_get(symbol=SYMBOL) or []:
        ...
        result = mt5.order_send(req)
        logging.info(f"Moved SL for ticket {pos.ticket} to {leg4_price}. Retcode={result.retcode}")
...
def place_next_recovery_grid():
    """
    Handles all recovery grid trades and SL placement logic.
    """
    global first_recovery_price, last_recovery_leg, last_recovery_price, last_recovery_side

    # Get all RECOVERY positions and find highest leg
    rec_positions = [p for p in (mt5.positions_get(symbol=SYMBOL) or []) if p.comment and p.comment.startswith("RECOVERY")]
    current_legs = [int(p.comment.split("_")[1]) for p in rec_positions if "_" in p.comment]
    last_recovery_leg = max(current_legs, default=1)

    if last_recovery_leg >= 3:
        return  # Only up to 3 recovery legs allowed

    next_leg = last_recovery_leg + 1
    grid_price = recovery_grid_price(first_recovery_price, lock_anchor_side, next_leg, STEP_PRICE)
    rec_side = lock_anchor_side if next_leg == 2 else ('SELL' if lock_anchor_side == 'BUY' else 'BUY')
    rec_lot = round(LOT_SIZE * next_leg, 2)
    comment = f"RECOVERY_{next_leg}"
...
def safe_pending(side, price, vol, comment):
    tick    = mt5.symbol_info_tick(SYMBOL)
    if not tick: return None
    market  = tick.ask if side == 'BUY' else tick.bid
    if abs(price - market) < min_pending_distance():
        logging.info(f"[SKIP‑PENDING] {comment} @{price:.3f} – too near market ({market:.3f})")
        return None                   # don’t send – wait
    return place_pending(side, price, vol, comment)
# watchlist helper
...
def sync_recovery_state():
    positions = mt5.positions_get(symbol=SYMBOL) or []
    rec_positions = [p for p in positions if p.comment and p.comment.startswith("RECOVERY")]
    if rec_positions:
        # Sort by leg order if possible, else by time
        comments = [p.comment for p in rec_positions]
        max_leg = max([int(c.split("_")[1]) for c in comments if "_" in c], default=1)
        last_pos = sorted(rec_positions, key=lambda x: x.time)[-1]
        global last_recovery_leg, last_recovery_side, last_recovery_price
        last_recovery_leg = max_leg
        last_recovery_side = 'BUY' if last_pos.type == mt5.POSITION_TYPE_BUY else 'SELL'
        last_recovery_price = last_pos.price_open
def cancel_all_recovery_pendings():
    """Cancel ALL pending RECOVERY orders for the symbol."""
    for o in mt5.orders_get(symbol=SYMBOL) or []:
        if o.comment and o.comment.startswith("RECOVERY"):
            delete_order(o.ticket)
def close_recovery_trade_and_update_recovered(pos):
    global recovered_so_far
    profit = pos.profit
    if profit > 0:
        recovered_so_far += profit
    close_position(pos, comment='REC_TP_EXIT')
def is_inside_lux_range(price, lux_ranges, now):
    """Returns True if price is inside any active LuxAlgo box at current time."""
    for r in lux_ranges:
...
def price_between_last_buy_sell(entry_sequence, price):
    """True if price is between last buy and last sell in entry_sequence."""
    buys  = [e[0] for e in entry_sequence if e[1] == 'BUY']
    sells = [e[0] for e in entry_sequence if e[1] == 'SELL']
    if not buys or not sells:
        return False
    low, high = sorted([min(buys), max(sells)])
    return low < price < high
def should_lock_trades(price, buy_price, sell_price, lux_ranges):
    now_utc = pd.Timestamp.utcnow().tz_localize('UTC')
    total_trades = len(mt5.positions_get()) + len([o for o in mt5.orders_get() if o.symbol == SYMBOL])

    if total_trades < 3:
        return False

    # Check price between last BUY and SELL
    low = min(buy_price, sell_price)
    high = max(buy_price, sell_price)
    in_middle = low <= price <= high

    # Check price inside any Lux range
    in_range = any(
        box['start'] <= now_utc <= box['end'] and box['low'] <= price <= box['high']
        for box in lux_ranges
    )

    return in_middle and in_range
...
def main():
    ...
    global lock_anchor_price, lock_anchor_side, base_entry_side
    global first_recovery_price, last_recovery_leg, last_recovery_price, last_recovery_side
    global recovered_so_far 
    # --- PATCH: reset recovery vars for new recovery cycle ---
    first_recovery_price = None
    last_recovery_leg = 0
    last_recovery_price = None
    last_recovery_side = None
    # ── INIT ────────────────────────────────────────────────────────────────
    ...
    while True:
        ...
        if not tick:
            ptime.sleep(1)
            continue
        bars_raw = mt5.copy_rates_from_pos(SYMBOL, TIMEFRAME, 0, 600)
        bars_df = pd.DataFrame(bars_raw) if bars_raw is not None and len(bars_raw) > 0 else pd.DataFrame()
        now_utc = pd.Timestamp.utcnow()  # Already UTC and tz-aware

        lux_ranges, lux_box, in_lux_box = [], None, False
        recent_box, is_breakout = None, False

        if not bars_df.empty and 'close' in bars_df and len(bars_df) >= LUX_ATR_LEN + LUX_LEN:
            lux_ranges = get_lux_ranges(bars_df)
            lux_box = current_lux_box(lux_ranges, now_utc)
            if lux_box and tick is not None:
                in_lux_box = (
                    (lux_box['bottom'] <= tick.bid <= lux_box['top'])
                    or
                    (lux_box['bottom'] <= tick.ask <= lux_box['top'])
                )
                logging.info(f"[LUX DEBUG] InBox={in_lux_box}, BoxTop={lux_box['top'] if lux_box else None}, BoxBottom={lux_box['bottom'] if lux_box else None}, Price={tick.bid if tick else None}")
            else:
                ended_boxes = [
                    r for r in lux_ranges
                    if (r['end'].tz_localize('UTC') if r['end'].tzinfo is None else r['end'].tz_convert('UTC')) < now_utc
                ]
                if ended_boxes:
                    recent_box = ended_boxes[-1]
                    if tick.bid > recent_box['top'] or tick.ask < recent_box['bottom']:
                        is_breakout = True
        else:
            logging.warning("[LUX] Not enough data to compute LuxAlgo consolidation box.")
        # ── Session & spread gate
        ...
        sync_pendings()
        sync_recovery_state()
        if not locked_active and in_lux_box and len(entry_sequence) >= 2:
            # If price is now consolidating inside trap and Lux range, lock immediately
            if price_between_last_buy_sell(entry_sequence, tick.bid):
                ...
                if lock_lot > 0.0:
                    ...
                    if hasattr(res, 'retcode') and res.retcode in SUCCESS:
                        ...
                        logging.info("Locked (fallback): trap + Lux range detected after entries.")
                        continue

        # Get open positions and calculate total PnL
        positions = mt5.positions_get(symbol=SYMBOL) or []
        total_pnl = sum(p.profit for p in positions)

        # --- FETCH LuxAlgo consolidation box (ATR range) ---
        ...
        # --- LOCK LOGIC: If inside Lux box AND inside trap zone ---
                # --- LOCK LOGIC (LuxAlgo) ---
        # Rule 1: If only 1 position open, lock immediately if price enters range
        if positions and len(positions) == 1 and not locked_active and in_lux_box:
            # Lock at current price, opposite side
            first_pos = positions[0]
            lock_side = 'SELL' if first_pos.type == mt5.POSITION_TYPE_BUY else 'BUY'
            lock_lot = first_pos.volume
            lock_price = tick.bid if lock_side == 'SELL' else tick.ask
            res = place_entry(lock_side, lock_price, lock_lot, "LUX_LOCK")
            if hasattr(res, 'retcode') and res.retcode in SUCCESS:
                locked_active = True
                ...
                lock_anchor_price = lock_price
                lock_anchor_side = lock_side
                cancel_all_pendings()  # <-- THIS cancels all pending orders
                logging.info("Locked (single position): entered LuxAlgo range. Waiting for lock fill.")
                continue


        # Rule 2: If more than 1 trade, and inside Lux range AND price is between last buy/sell, lock all
        if positions and len(positions) > 1 and not locked_active and in_lux_box and price_between_last_buy_sell(entry_sequence, tick.bid):
    # locking logic
            ...
            if lock_lot > 0.0:
                ...
                if hasattr(res, 'retcode') and res.retcode in SUCCESS:
                    ...
                    lock_anchor_side = lock_side
                    cancel_all_pendings()  # <-- THIS cancels all pending orders
                    logging.info("Locked (trap): multi-trade, price inside trap and LuxAlgo range. Waiting for lock fill.")
                    continue
        ...
        if lock_order_ticket is not None:
            ...
            if orders is not None and len(orders) == 0:
                ...
                # --- FIX: Add this line before logging ---
                recovery_pnl = get_recovery_pnl()

                logging.info(f"[RECOVERY STATUS] Locked Loss: {locked_loss:.2f}, Current Recovery PnL: {recovery_pnl:.2f}, Remaining: {locked_loss - recovery_pnl:.2f}")
                # ...
        ...
        # --- BLOCK entries if price is in LuxAlgo range ---
        if not positions and not locked_active:
            if bars_df.empty or len(bars_df) < 30:
                logging.info("[LUX BLOCK] Not enough bars to determine LuxAlgo range. Blocking entry.")
                ptime.sleep(1)
                continue
            elif lux_box is not None:
                if in_lux_box:
                    logging.info("[LUX BLOCK] Price inside LuxAlgo range – skipping entry.")
                    ptime.sleep(1)
                    continue
                else:
                    logging.info("[LUX BLOCK] Price outside LuxAlgo range – allowed to check entry.")
                    # allowed, proceed to entry check
            elif recent_box is not None:
                if is_breakout:
                    logging.info("[LUX BREAKOUT] Price has just broken out of last LuxAlgo range. Entry allowed.")
                    # allowed, proceed to entry check
                else:
                    logging.info("[LUX BREAKOUT BLOCK] Price not clearly outside last LuxAlgo range. Blocking entry.")
                    ptime.sleep(1)
                    continue
            else:
                logging.info("[LUX BLOCK] No LuxAlgo box detected (no recent box either). Blocking entry for safety.")
                ptime.sleep(1)
                continue
                ...
                if is_high_confidence_entry(skew, spread, vol_ok, mom_ok, is_consol, strong, poc_drift):
                    ...
                    if hasattr(res, 'retcode') and res.retcode == mt5.TRADE_RETCODE_DONE:
                        ...
                        # Leg 2 (opposite side) pending
                                        # After entry, check if price is inside LuxAlgo range
                        # After placing GRID_1_MARKET and appending to entry_sequence:
                        # After placing GRID_1_MARKET and appending to entry_sequence:
                        if in_lux_box:
                            logging.info("[LUX LOCK] Entry made inside LuxAlgo box. Locking immediately and skipping grid pending.")
                            lock_side = 'SELL' if side == 'BUY' else 'BUY'
                            lock_lot = LOT_SIZE
                            lock_price = tick.bid if lock_side == 'SELL' else tick.ask
                            res_lock = place_entry(lock_side, lock_price, lock_lot, "LUX_LOCK")
                            if hasattr(res_lock, 'retcode') and res_lock.retcode in SUCCESS:
                                locked_active = True
                                lock_order_ticket = getattr(res_lock, 'order', None)
                                lock_anchor_price = lock_price
                                lock_anchor_side = lock_side
                                cancel_all_pendings()
                                logging.info("Locked (single position): entered LuxAlgo range at entry. No pending orders placed.")
                                continue  # Go to next loop iteration



                        # Otherwise, only place grid pending if not in range
                        else:
                            opp = next_grid_side(base_entry_side, 2)
                            leg2_price = entry_price - STEP_PRICE if base_entry_side == 'BUY' else entry_price + STEP_PRICE
                            res2 = safe_pending(opp, leg2_price, LOT_SIZE*2, "GRID_2_PENDING")
                            track_pending(res2, "GRID_2_PENDING", opp)
        ...
        if current_leg == 2:
            side3 = next_grid_side(base_entry_side, 3)   # same side as base

            if not already_pending("GRID_3_PENDING", side3):          # ← NEW GUARD
                tick_price = tick.ask if side3 == 'BUY' else tick.bid

                if abs(tick_price - base_entry_price) <= min_pending_distance():
                    res = place_entry(side3, tick_price, LOT_SIZE*3, "GRID_3_MARKET")
                    if res.retcode == mt5.TRADE_RETCODE_DONE:
                        entry_sequence.append((tick_price, side3, LOT_SIZE*3))
                        current_leg = 3
                        logging.info("[GRID‑3] executed at market")
                else:
                    offset = min_pending_distance() * 1.1
                    grid_price = (
                        base_entry_price - offset if side3 == 'BUY'
                        else base_entry_price + offset
                    )
                    res3 = safe_pending(side3, grid_price, LOT_SIZE*3, "GRID_3_PENDING")
                    track_pending(res3, "GRID_3_PENDING", side3)      # ← keeps watchlist
        ...
        if locked_active and post_lock_recovery:
            ...
            total_recovery = recovered_so_far + get_recovery_pnl()  # sum of realized + unrealized
            if total_recovery > locked_loss:
                logging.info(f"[RECOVERY SUCCESS] Recovered locked loss. Total recovered: {total_recovery:.2f}. Closing all trades and resetting.")
                log_trade(entry_sequence)
                close_everything()
                cancel_all_recovery_pendings()
                recovered_so_far = 0.0  # Reset after full recovery
                reset_state()
                continue


            if rec_positions and recovery_pnl >= PROFIT_SCALP_TARGET:
                logging.info("[RECOVERY TP HIT] Closing recovery trades and pending recovery orders.")
                for pos in rec_positions:
                    close_recovery_trade_and_update_recovered(pos)
                cancel_all_recovery_pendings()
                ptime.sleep(2)
                continue


            if not rec_positions and locked_loss > 0:
                # Calculate confirmations before entering RECOVERY_1
                from_time = datetime.now(timezone.utc) - timedelta(minutes=5)
                ticks = mt5.copy_ticks_from(SYMBOL, from_time, 3000, mt5.COPY_TICKS_ALL)
                df_ticks = pd.DataFrame(ticks)
                bars = mt5.copy_rates_from_pos(SYMBOL, TIMEFRAME, 0, 100)
                ...
                if not df_ticks.empty:
                    # SKIP if price is inside LuxAlgo consolidation range
                    if in_lux_box:
                        logging.info("[RECOVERY BLOCKED] Price inside LuxAlgo range during recovery. Waiting to exit range.")
                        ptime.sleep(1)
                        continue

                    skew, _, poc_drift = calc_skew(df_ticks, return_poc=True)
                    vol_ok = df_ticks['volume'].iloc[-1] > df_ticks['volume'].mean()
                    mom_ok = is_momentum_candle(closes, highs, lows, volumes)
                    is_consol = detect_consolidation(closes)[1]
                    strong = is_strong_candle(closes, highs, lows)
                    score = get_confirmation_score(skew, spread, vol_ok, mom_ok, is_consol, strong, poc_drift)
                    if score < 5:
                        logging.info(f"[RECOVERY_1 BLOCKED] Confirmation score only {score}/7. Waiting for better conditions.")
                        ptime.sleep(1)
                        continue  # Wait for better confirmation
                    ...
                    # --- PATCH: Always set recovery anchor at first recovery trade ---
                    first_recovery_price = rec_price
                    last_recovery_leg = 1
                    last_recovery_price = rec_price
                    last_recovery_side = rec_side

                    logging.info(f"[RECOVERY INIT] Placing first recovery trade: {rec_side} {rec_lot} @ {rec_price} (locked_loss: {locked_loss}), confirmation score={score}/7")
            ...
            if 0 < last_recovery_leg < 3:
                if in_lux_box:
                    logging.info(f"[RECOVERY-GRID BLOCKED] in_lux_box={in_lux_box}. Will not place pending recovery grid until price exits consolidation range.")
                    ptime.sleep(1)
                    continue
            ...
            # After RECOVERY_3 filled, move all SLs to RECOVERY_4 grid price ("trap" SL logic)
            # After RECOVERY_3 filled, move all SLs to RECOVERY_4 grid price ("trap" SL logic)
            if last_recovery_leg == 3:
                logging.info("[RECOVERY SL UPDATE] 3rd recovery filled; moving all SLs to 4th grid price.")
                move_all_recovery_sls_to_leg4_price()
                # --- Check for SL hit at 4th grid price (panic exit) ---
                leg4_price = recovery_grid_price(first_recovery_price, lock_anchor_side, 4, STEP_PRICE)
                curr_bid = tick.bid
                curr_ask = tick.ask
                panic_exit = False
                if lock_anchor_side == 'BUY' and min(curr_bid, curr_ask) <= leg4_price:
                    panic_exit = True
                elif lock_anchor_side == 'SELL' and max(curr_bid, curr_ask) >= leg4_price:
                    panic_exit = True
                if panic_exit:
                    logging.warning("[PANIC EXIT] Price hit RECOVERY_4 grid; closing all recovery positions.")
                    log_trade(entry_sequence)
                    for pos in rec_positions:
                        close_position(pos, comment='REC_PANIC_EXIT')
                    close_everything() 
                    reset_state()
                    continue
            ...
            # If all recovery trades closed (either by TP or SL), will re-enter as needed in the next loop.


            # elif last_recovery_leg >= 3:  # (NO! now handled by panic exit logic)
